...
const EMPTY_ASSET = { name: "", assetType: "character", storageUri: "", mimeType: "", notes: "", parentAssetId: "", reusable: true, fileName: "", fileSize: 0 };
...
export default function ProjectLibrary() {
  ...
  const selectAssetFile = (event) => {
    ...
    reader.onload = () => {
      ...
      setAssetForm((form) => ({ ...form, storageUri: String(reader.result || ""), mimeType: file.type || "application/octet-stream", fileName: file.name, fileSize: file.size, assetType: tab === "Images & Video" ? detectedType : form.assetType }));
  ...
  const createAsset = () => run(async () => {
    await request("/api/creator-library/assets", { method: "POST", body: JSON.stringify({ ...assetForm, source: assetForm.fileName ? "upload" : "user", projectId: activeId, parentAssetId: assetForm.parentAssetId || null, metadata: { notes: assetForm.notes, fileName: assetForm.fileName, fileSize: assetForm.fileSize } }) });
