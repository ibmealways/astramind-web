...
const EMPTY_ASSET = { name: "", assetType: "character", storageUri: "", mimeType: "", notes: "", parentAssetId: "", reusable: true, fileName: "", fileSize: 0, generatedBy: "", referenceAssetId: "", referenceStrength: 0.45, generationSeed: -1, lockGenerationSeed: false };
...
function blobToDataUrl(blob) {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => reject(new Error("Generated image could not be prepared for the library."));
...
export default function ProjectLibrary() {
  ...
  const loadProjects = useCallback(async () => {
    ...
    setActiveId((current) => available.some((project) => project.id === current) ? current : available[0]?.id || "");
  }, []);
  ...
  useEffect(() => {
    const recoverableAssetForm = assetForm.fileName ? { ...assetForm, storageUri: "", fileName: "", fileSize: 0 } : assetForm;
    const draft = { activeId, tab, projectForm, episodeForm, sceneForm, assetForm: recoverableAssetForm, savedAt: new Date().toISOString() };
  ...
  const referenceAssets = (library?.assets || []).filter((asset) => ["character", "location", "image"].includes(asset.assetType) && /^data:image\/(png|jpeg);base64,/i.test(asset.storageUri || ""));
  const selectAssetFile = (event) => {
    const file = event.target.files?.[0];
    if (!file) return;
    if (file.size > MAX_UPLOAD_BYTES) {
      event.target.value = "";
      setStatus("File is larger than the 8 MB library upload limit.");
      return;
    }
    const reader = new FileReader();
    reader.onload = () => {
      const detectedType = file.type.startsWith("video/") ? "video" : file.type.startsWith("audio/") ? "voice" : assetTypes[0];
      setAssetForm((form) => ({ ...form, storageUri: String(reader.result || ""), mimeType: file.type || "application/octet-stream", fileName: file.name, fileSize: file.size, generatedBy: "", assetType: tab === "Images & Video" ? detectedType : form.assetType }));
      setStatus(`${file.name} is ready to save with this asset.`);
    };
  ...
  const generateAssetImage = async () => {
    ...
    setStatus("Aigenikz is generating the reference image on the connected PC…");
    try {
      const subject = tab === "Characters"
        ? `SOLO CHARACTER ONLY. Exactly one person in the entire image. Individual anime character reference for ${assetForm.name}. ${assetForm.notes || "Create a distinctive young adult supernatural adventure hero."} Isolate the named character from the visual reference. Full-body three-quarter pose, clear unobstructed face, neutral readable background, consistent costume and power design. No companion, no group, no second person, no split-screen, no character sheet.`
        : `Original cinematic anime environment reference for ${assetForm.name}. ${assetForm.notes || "Create a distinctive supernatural adventure location."} Wide establishing view, clear geography, reusable production reference.`;
      const referenceAsset = referenceAssets.find((asset) => asset.id === assetForm.referenceAssetId);
      const generated = await request("/api/ai-image/generate", { method: "POST", body: JSON.stringify({ prompt: subject, style: "cinematic anime fantasy", aspect: tab === "Characters" ? "portrait" : "landscape", seed: assetForm.lockGenerationSeed ? Number(assetForm.generationSeed) : -1, referenceImage: referenceAsset?.storageUri || null, referenceStrength: Number(assetForm.referenceStrength) }) });
      const imageResponse = await fetch(generated.publicUrl);
      if (!imageResponse.ok) throw new Error("Generated image could not be retrieved from Aigenikz.");
      const imageBlob = await imageResponse.blob();
      if (imageBlob.size > MAX_UPLOAD_BYTES) throw new Error("Generated image exceeds the 8 MB library limit.");
      const dataUrl = await blobToDataUrl(imageBlob);
      setAssetForm((form) => ({ ...form, storageUri: dataUrl, mimeType: imageBlob.type || "image/png", fileName: `${form.name.trim().replace(/[^a-z0-9]+/gi, "-").toLowerCase()}-aigenikz.png`, fileSize: imageBlob.size, generatedBy: generated.model || "aigenikz-local", generationSeed: generated.seed ?? form.generationSeed }));
      setStatus(`Aigenikz generated ${assetForm.name}. Review it, then click Add to library.`);
    } catch (error) {
      setStatus(error.message || "Aigenikz image generation failed.");
    } finally {
      setGeneratingAsset(false);
    }
  };
  const createAsset = () => run(async () => {
    await request("/api/creator-library/assets", { method: "POST", body: JSON.stringify({ ...assetForm, source: assetForm.generatedBy ? "aigenikz-local" : assetForm.fileName ? "upload" : "user", projectId: activeId, parentAssetId: assetForm.parentAssetId || null, metadata: { notes: assetForm.notes, fileName: assetForm.fileName, fileSize: assetForm.fileSize, generatedBy: assetForm.generatedBy, referenceAssetId: assetForm.referenceAssetId || null, referenceStrength: assetForm.referenceStrength, generationSeed: assetForm.generationSeed } }) });
  ...
  const discardAssetCandidate = () => {
    setAssetForm((form) => ({ ...form, storageUri: "", mimeType: "", fileName: "", fileSize: 0, generatedBy: "" }));
    setStatus("Generated candidate discarded. You can generate another version.");
  };
    ...
    <div className="library-layout">
      ...
      <main className="library-workspace">
        ...
        {activeId && <>
          ...
          {["Characters", "Locations", "Voices", "Images & Video"].includes(tab) && <div className="library-assets-layout"><section className="library-panel"><div className="library-panel-heading"><div><span>Reusable production memory</span><h2>Add {tab === "Images & Video" ? "media" : tab.toLowerCase()}</h2></div></div><div className="library-form-stack"><label>Name<input value={assetForm.name} onChange={(e) => setAssetForm({ ...assetForm, name: e.target.value })} placeholder={tab === "Characters" ? "Kyle" : tab === "Locations" ? "Glowing cave pond" : tab === "Voices" ? "Kyle — warm tenor" : "Graduation courtyard reference"} /></label>{tab === "Images & Video" && <label>Media type<select value={assetForm.assetType} onChange={(e) => setAssetForm({ ...assetForm, assetType: e.target.value, mimeType: e.target.value === "video" ? "video/mp4" : "image/png" })}><option value="image">Image</option><option value="video">Video clip</option></select></label>}<label>Direct asset URL (optional)<input value={assetForm.fileName ? "" : assetForm.storageUri} disabled={Boolean(assetForm.fileName)} onChange={(e) => setAssetForm({ ...assetForm, storageUri: e.target.value })} placeholder="https://…" /></label><div className="library-upload-divider"><span>or upload from this device</span></div><label className="library-upload-control"><input type="file" accept={tab === "Voices" ? "audio/*" : tab === "Images & Video" ? "image/*,video/*" : "image/*"} onChange={selectAssetFile} /><span>Choose image or file</span><small>Stored with this asset · maximum 8 MB</small></label>{["Characters", "Locations"].includes(tab) && <><div className="library-upload-divider"><span>or create inside Aigenikz</span></div>{referenceAssets.length > 0 && <div className="library-reference-controls"><label>Visual reference<select value={assetForm.referenceAssetId} onChange={(e) => setAssetForm({ ...assetForm, referenceAssetId: e.target.value })}><option value="">Text only — no reference</option>{referenceAssets.map((asset) => <option key={asset.id} value={asset.id}>{asset.name} · v{asset.version}</option>)}</select></label>{assetForm.referenceAssetId && <><label>Reference influence <strong>{Math.round(Number(assetForm.referenceStrength) * 100)}%</strong><input type="range" min="0.35" max="0.9" step="0.05" value={assetForm.referenceStrength} onChange={(e) => setAssetForm({ ...assetForm, referenceStrength: Number(e.target.value) })} /></label><small>Use 40–50% to extract one person from a group image; use about 70% with an individual reference.</small>{Number(assetForm.generationSeed) >= 0 && <label className="library-check"><input type="checkbox" checked={Boolean(assetForm.lockGenerationSeed)} onChange={(e) => setAssetForm({ ...assetForm, lockGenerationSeed: e.target.checked })} />Reuse seed {assetForm.generationSeed} for the same composition</label>}</>}</div>}<button type="button" className="library-generate-button" disabled={generatingAsset || !assetForm.name.trim()} onClick={generateAssetImage}>{generatingAsset ? "Generating on connected PC…" : assetForm.referenceAssetId ? "✦ Generate from reference" : "✦ Generate with Aigenikz"}</button></>}{assetForm.fileName && <div className="library-selected-file"><div><strong>{assetForm.fileName}</strong><small>{(assetForm.fileSize / 1024 / 1024).toFixed(2)} MB · {assetForm.mimeType}</small></div><button type="button" onClick={discardAssetCandidate}>Remove</button></div>}<label>Create new version of<select value={assetForm.parentAssetId} onChange={(e) => setAssetForm({ ...assetForm, parentAssetId: e.target.value })}><option value="">New master asset</option>{visibleAssets.map((asset) => <option key={asset.id} value={asset.id}>{asset.name} · v{asset.version}</option>)}</select></label><label>Continuity notes<textarea value={assetForm.notes} onChange={(e) => setAssetForm({ ...assetForm, notes: e.target.value })} placeholder="Appearance, wardrobe, voice, behavior, camera or continuity details…" /></label><label className="library-check"><input type="checkbox" checked={assetForm.reusable} onChange={(e) => setAssetForm({ ...assetForm, reusable: e.target.checked })} />Reusable across scenes and episodes</label><button className="library-primary" disabled={busy || generatingAsset || !assetForm.name.trim()} onClick={createAsset}>{assetForm.parentAssetId ? "Save new version" : "Add to library"}</button></div></section><section><div className="library-asset-header"><div><h2>{assetForm.storageUri ? "Candidate preview" : tab}</h2><p>{assetForm.storageUri ? "Review before saving" : `${visibleAssets.length} saved assets`}</p></div></div>{assetForm.storageUri && <article className="library-candidate-card"><div className="library-candidate-preview"><AssetPreview asset={assetForm} /></div><div className="library-candidate-copy"><span>{assetForm.generatedBy ? "Generated by Aigenikz" : "Selected file"}</span><h3>{assetForm.name || assetForm.fileName}</h3><p>{assetForm.notes || "Review this reference before adding it to the permanent library."}</p><div><button className="library-primary" disabled={busy || !activeId} onClick={createAsset}>Keep and add to library</button><button className="library-discard-button" onClick={discardAssetCandidate}>Discard</button></div></div></article>}<div className="library-asset-grid">{visibleAssets.map((asset) => <article key={asset.id} className="library-asset-card"><div className="library-asset-preview"><AssetPreview asset={asset} /><span>v{asset.version}</span></div><div><strong>{asset.name}</strong><small>{asset.assetType} · {asset.source}{asset.metadata?.fileName ? ` · ${asset.metadata.fileName}` : ""}</small><p>{asset.metadata?.notes || "No continuity notes yet."}</p>{asset.parentAssetId && <em>Versioned from an earlier asset</em>}</div></article>)}{!assetForm.storageUri && !visibleAssets.length && <div className="library-empty-state"><span>＋</span><h3>No {tab.toLowerCase()} saved yet</h3><p>Generate or upload a candidate here, then decide whether to keep it.</p></div>}</div></section></div>}
        </>}
