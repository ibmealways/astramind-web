...
const EMPTY_ASSET = { name: "", assetType: "character", storageUri: "", mimeType: "", notes: "", parentAssetId: "", reusable: true, fileName: "", fileSize: 0, generatedBy: "" };
  ...
  const generateAssetImage = async () => {
    ...
    try {
      const subject = tab === "Characters"
        ? `Single original anime character reference for ${assetForm.name}. ${assetForm.notes || "Create a distinctive young adult supernatural adventure hero."} Full-body three-quarter pose, clear face, neutral readable background, consistent costume and power design, one character only.`
      ...
      const generated = await request("/api/ai-image/generate", { method: "POST", body: JSON.stringify({ prompt: subject, style: "cinematic anime fantasy", aspect: tab === "Characters" ? "portrait" : "landscape" }) });
      ...
      setAssetForm((form) => ({ ...form, storageUri: dataUrl, mimeType: imageBlob.type || "image/png", fileName: `${form.name.trim().replace(/[^a-z0-9]+/gi, "-").toLowerCase()}-aigenikz.png`, fileSize: imageBlob.size, generatedBy: generated.model || "aigenikz-local" }));
