...
export default function ContentVideo() {
  ...
  const generateVideoPlan = async (event) => {
    ...
    if (!cleanTopic) {
      setNotice("  Enter a video topic first.");
      return;
    }
    ...
    const response = await fetch(`${API_URL}/api/cinematic-video/${background ? "render-background" : "render"}`, {
      ...
      body: JSON.stringify({
        ...
        options: {
          ...
          stock: videoMode === "aigenikz-local" ? false : licensedStock,
          mediaStrategy: videoMode === "aigenikz-local" ? "generated" : "hybrid",
          subtitles,
...
if (!rawVideoUrl) {
  ...
  const exportCaptions = () => {
    ...
    if (!captions?.length) {
      setNotice("  No captions found to export yet.");
      return;
    }
